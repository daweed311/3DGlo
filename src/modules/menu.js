...
const menu = () => {
  ...
  const smoothScroll = (href) => {
    ...
    const scrollTarget =
      Math.round(transitionElement.getBoundingClientRect().top) + scrollY;
    ...
    (function animation() {
      scrollY += stepScroll;
      if (scrollY < scrollTarget || scrollY - scrollTarget < stepScroll) {
  ...
  document.querySelector("body").addEventListener("click", (e) => {
    ...
    if (itemMenu) {
      smoothScroll(itemMenu.getAttribute("href"));
    }
  });
};
